...
async function englishProductIntro(env: Env, request: Request, id: number): Promise<Response> {
  ...
  if (!product) return errorResponse('Product not found.', 404);
  try {
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${encodeURIComponent(env.GEMINI_API_KEY)}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-goog-api-key': env.GEMINI_API_KEY }, body: JSON.stringify({ contents: [{ parts: [{ text: `Write a plain, factual English product introduction in no more than three sentences. Use only this product name and description. Do not invent origin, ingredients, certifications, reviews, measurements, or other facts. Product name: ${product.name}\nDescription: ${product.description}` }] }], generationConfig: { temperature: 0.2, maxOutputTokens: 120 } }) });
    if (!response.ok) { const failure = await response.text(); console.error('Gemini API error', response.status, failure.slice(0, 500)); return errorResponse('English introduction was unavailable.', 502); }
