...
declare const daum: { Postcode: new (options: { oncomplete: (data: { zonecode: string; roadAddress: string; jibunAddress: string }) => void }) => { open: () => void } } | undefined;
...
async function renderCart(): Promise<void> {
  ...
  document.querySelector<HTMLButtonElement>('#postcode-search')?.addEventListener('click', () => { if (!daum?.Postcode) { document.querySelector('#cart-notice')!.textContent = '우편번호 검색을 불러오지 못했습니다.'; return; } new daum.Postcode({ oncomplete: (data) => { document.querySelector<HTMLInputElement>('#postal-code')!.value = data.zonecode; document.querySelector<HTMLInputElement>('#base-address')!.value = data.roadAddress || data.jibunAddress; document.querySelector<HTMLInputElement>('#detail-address')?.focus(); } }).open(); });
