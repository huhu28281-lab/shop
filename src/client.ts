...
async function renderCart(): Promise<void> {
  ...
  document.querySelector<HTMLButtonElement>('#postcode-search')?.addEventListener('click', () => { if (!daum?.Postcode) { document.querySelector('#cart-notice')!.textContent = '우편번호 검색을 불러오지 못했습니다. 페이지를 새로고침해 주세요.'; return; } const layer = document.createElement('div'); layer.className = 'modal-backdrop postcode-layer-backdrop'; layer.innerHTML = '<div class="postcode-layer" role="dialog" aria-modal="true" aria-label="우편번호 검색"><button type="button" class="map-close" aria-label="닫기">닫기</button><div id="postcode-embed"></div></div>'; document.body.appendChild(layer); const postcode = new daum.Postcode({ oncomplete: (data) => { document.querySelector<HTMLInputElement>('#postal-code')!.value = data.zonecode; document.querySelector<HTMLInputElement>('#base-address')!.value = data.roadAddress || data.jibunAddress; document.querySelector<HTMLInputElement>('#detail-address')?.focus(); layer.remove(); } }); postcode.embed(document.querySelector('#postcode-embed')!); layer.querySelector('.map-close')?.addEventListener('click', () => layer.remove()); layer.addEventListener('click', (event) => { if (event.target === layer) layer.remove(); }); });
