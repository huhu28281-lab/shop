...
async function renderHome(category = new URLSearchParams(location.search).get('category') ?? ''): Promise<void> {
  ...
  app.innerHTML = shell('home', `<main class="page"><div class="catalog-layout">${sidebar}<section class="catalog-content"><div class="breadcrumb">${text.products}</div><h1 class="page-heading">${text.product}</h1><nav class="category-tabs" aria-label="${text.product}">${tabs}</nav><div class="listing-meta">${visible.length}${text.count}</div><section class="product-grid" aria-label="${text.products}">${visible.map((product) => { const isHit = !firstByCategory.has(product.category); firstByCategory.add(product.category); return productCard(product, lang, isHit); }).join('')}</section></section></div></main>`, true);
