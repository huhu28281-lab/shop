...
async function renderProduct(id: number): Promise<void> {
  ...
  let qty = 1;
  const detailSection = copy.description ? `<section class="detail-extra"><h2>상품 상세 정보</h2><div class="detail-copy"><p>${escapeHtml(copy.description)}</p></div><dl class="detail-specs"><div><dt>분류</dt><dd>${escapeHtml(copy.category)}</dd></div><div><dt>판매 가격</dt><dd>${won(data.product.price)}</dd></div></dl></section>` : '';
