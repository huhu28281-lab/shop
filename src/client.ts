...
async function renderMyPage(): Promise<void> {
  try {
    ...
    const orders = await request<{ orders: Array<{ id: string; total: number; status: string; shipping_status: string; created_at: string; items: Array<{ product_name: string; qty: number; shoe_size?: number | null }> }> }>('/api/orders');
    const orderRows = orders.orders.length ? orders.orders.map((order) => {
      ...
      const shippingLabel = order.shipping_status === 'preparing' ? '배송 준비 중' : '결제 대기';
    ...
    const shippingRows = orders.orders.length ? orders.orders.map((order) => {
      const shippingLabel = order.shipping_status === 'preparing' ? '배송 준비 중' : '결제 후 배송이 시작됩니다';
...
async function renderOrder(id: string): Promise<void> {
  ...
  const shippingPanel = `<section class="virtual-shipping"><h2>배송 조회</h2><p><strong data-shipping-status>${shipping.status === 'paid' ? '배송 준비 중' : '결제 후 배송이 시작됩니다'}</strong></p><p>현재 위치: <span data-shipping-location>${escapeHtml(shipping.current_location ?? '배송 정보 대기')}</span></p>${shipping.shipping_address ? `<p>배송지: ${escapeHtml(shipping.shipping_address)}</p>` : ''}<small>가상 배송 상태이며 실제 택배 위치와는 다를 수 있습니다.</small></section>`;
  const lang = language(); const text = ui[lang];
