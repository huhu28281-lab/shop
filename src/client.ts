...
async function renderCart(): Promise<void> {
  ...
  document.querySelector<HTMLButtonElement>('#order-button')?.addEventListener('click', async () => { const form = document.querySelector<HTMLFormElement>('#shipping-form')!; if (!form.reportValidity()) return; try { const result = await request<{ order: Order }>('/api/orders', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(form).entries())) }); go(`/checkout/${result.order.id}`); } catch (error) { document.querySelector('#cart-notice')!.textContent = (error as Error).message; } });
}
